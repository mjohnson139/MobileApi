...
class ReduxStateDemo {
  ...
  logCurrentState() {
    ...
    console.log(
      `  UI: Screen=${state.ui.currentScreen}, Sidebar=${
        state.ui.components.sidebar.isOpen ? 'open' : 'closed'
      }`,
    );
    console.log(
      `  Devices: ${Object.keys(state.devices.devices).length} devices, ${
        state.devices.metrics.totalStateUpdates
      } updates`,
    );
    console.log(
      `  Server: ${state.server.isRunning ? 'running' : 'stopped'}, ${
        state.server.requests.total
      } requests`,
    );
  }
