...
class PerformanceBenchmark {
  ...
  async benchmarkThroughput() {
    ...
    for (const concurrency of concurrent_requests) {
      ...
      try {
        ...
        console.log(
          `    ${concurrency} requests completed in ${totalTime}ms (${
            this.results.throughput[`${concurrency}_concurrent`].requests_per_second
          } req/s)`,
        );
      } catch (error) {
        this.results.throughput[`${concurrency}_concurrent`] = {
  ...
  generateReport() {
    ...
    console.log(
      `   Range: ${this.results.startup_time.min}ms - ${this.results.startup_time.max}ms`,
    );
    ...
    const passedTests =
      (this.results.startup_time.passed ? 1 : 0) +
